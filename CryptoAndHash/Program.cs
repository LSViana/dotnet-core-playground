...
namespace CryptoAndHash
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Hash.Run();
            Console.WriteLine();
            // Keyed hashes provide a single way to transform data using a single secret key, which can be used to verify authenticity and data integrity. In this case, the generated hash (also called MAC) must be supplied with the original message, but the key used in generation is kept hidden, it is not useful to hide the message content.
            KeyedHash.Run();
            Console.WriteLine();
